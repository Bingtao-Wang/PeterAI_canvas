...
- PeterAI 接入：验证 iframe JWT 初始化后地址清理、刷新 session 恢复、恶意 `src_host` 拒绝和 401 会话清理。
- PeterAI 数据隔离：使用同一浏览器依次登录两个用户，确认画布、素材、媒体、生成记录、WebDAV 和 Agent 配置互不串读，持久化数据中不存在 PeterAI Key/JWT。
- PeterAI 多能力渠道：为 Sub2API 账号显式开启 `audio_speech` / `seedance` 能力并配置独立价格后，验证全部有效 Key 的模型交集导入、图片/文本/Grok/Seedance/音频分类，以及单 Key 模型加载失败不阻塞其他 Key；选择 PeterAI 托管模型时请求和扣费必须只进入 Sub2API，用户手工 URL/API Key 渠道仍按原直连行为工作。
- PeterAI 生图：验证动态 1K/2K/4K 预计价格、未知价格提示、每请求 `n=1`、最多四并发、部分失败和 Gallery 发布。
- PeterAI Grok 视频：验证 JSON 创建请求的时长、分辨率、单张参考图、`request_id` 轮询以及 `video.url` 下载/播放；多参考图应在请求前给出明确提示。
- PeterAI 部署：验证 `canvas.peterai.cc.cd` iframe、安全响应头、100MB 参考图、120 秒代理超时、代理白名单、Cloudflare Tunnel 和回滚。
- PeterAI 网站 Agent：右侧面板恢复“网站 / 本机”切换；网站模式应显示文本模型实际所属渠道、默认文本模型和“配置 URL / API Key”入口，嵌入页与新窗口都必须能打开配置弹窗；仅包含生图模型的托管渠道不得被选成文本渠道。PeterAI 域名请求必须走同源 `/peter-api/v1/responses` 并计入站内 usage，手工第三方渠道保持浏览器直连；验证工具确认/拒绝、画布操作、站点工具、停止请求和用户隔离的模式偏好。
...
- 全站 Agent：网站模式恢复前端 Responses 工具循环，本机模式继续走 MCP + 本地 Codex；两种模式切换时对话和连接配置不得混用。
...
- 画布 Agent：右侧 Agent 面板恢复网站 / 本机模式切换，默认使用网站模式并按 PeterAI 用户命名空间记住选择。
