...
- PeterAI 多能力渠道：开启 Sub2API `available_channels_enabled` 后验证全部有效 Key 的模型交集导入、图片/文本/Grok 视频分类，以及单 Key 模型加载失败不阻塞其他 Key；音频和 Seedance 在 Sub2API 增加真实路由前仅验证手工第三方渠道。
