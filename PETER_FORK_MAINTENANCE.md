...
当前生产构建为源码 `4df6198ede6f`、镜像 `peterai-canvas:20260712-upstream-ebd8ae2-4df6198e`。`custom/peterai` 已推送到公共仓库，精确 commit URL 已验证可公开访问；本机与公网健康、安全头、查询字符串脱敏、代理白名单、源码链接、DNS/Tunnel、正式菜单、iframe URL 和 CSP 均已验收。Agent 已恢复“网站 / 本机”切换，网站模式可直接打开渠道 URL/API Key 配置并使用文本模型；配置弹窗在 iframe 与新窗口均有全局挂载，图片/文本/视频/音频的托管能力和默认模型严格隔离。手工填写 PeterAI API 域名会使用同源 `/peter-api`，嵌入页不再默认连接可选本地 Agent。`available_channels_enabled` 当前仍关闭：图片模型可从生图选项安全导入，非图片托管模型要等真实账号、显式能力和价格验收后再开放。
